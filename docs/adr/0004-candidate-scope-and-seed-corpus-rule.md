...
- **Candidate is a dated role.** It starts on the reference listing and ends with a reason (withdrawal, primary lost, absent from the official list, dropped from the references). An ended role keeps its profile online, labelled with the end date; imports for that person stop. Nothing is deleted when a primary is lost.
...
- **No balance audit.** The method page states the selection rule and nothing more; it publishes no per-Candidate counts or speaking time.
  - **Amendment, 2026-09-07 (data-model ticket #14).** The original consequence read: "The balance audit is a distribution, not a target. The method page publishes, per Candidate, the number of Confrontations they took part in and their speaking time summed from Segments, and names the Candidates with none." Patrick withdrew it: the site exists to analyse what Candidates say, not to measure their airtime, and proving neutrality with counts is a distraction. Non-partisanship now rests on the mechanical selection rule alone, stated once on the method page, and on equal treatment of every Candidate in display and depth. The map's invariant 1 and `docs/launch-checklist.md` were amended the same day.
